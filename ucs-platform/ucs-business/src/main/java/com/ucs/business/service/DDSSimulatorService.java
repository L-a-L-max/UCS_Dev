...
/**
 * Simulates DDS drone telemetry data generation (dev/test mode only).
 * Replaces real DDS/PX4/MAVLink network for frontend integration testing.
 *
 * Generates position data for all registered drones at configured intervals,
 * then delegates to:
 *   - PartitionRoutingService (partition lookup)
 *   - TelemetryPersistenceService (batch DB writes)
 *   - WebSocketGatewayService (broadcasts to frontend via STOMP/WebSocket)
 *
 * Enable via: dds.simulator.enabled=true (default in dev profile)
 * Disable for production or when using real DDS/MAVLink gateways.
 */
@Slf4j
@Service
...
public class DDSSimulatorService {
    ...
    private void updateDronePosition(DroneSimState state) {
        ...
        double speedMs = state.groundSpeed;
