  ...
  return (
    <div className="h-screen bg-slate-900 text-white flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* 左侧: 无人机列表 */}
        <div className="w-64 bg-slate-800 border-r border-slate-700 overflow-y-auto p-2 space-y-1">
        ...
        {/* 中间: 控制面板 */}
        <div className="w-[360px] min-w-[300px] overflow-y-auto p-3 border-r border-slate-700">
          {!selectedDrone ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-slate-500">
                <Plane className="w-12 h-12 mx-auto mb-3 opacity-20" />
                <p className="text-sm">请从左侧选择无人机</p>
                <p className="text-xs mt-1">选择后可进行控制操作</p>
              </div>
            </div>
          ) : (
                ...
                <Card className="bg-slate-800 border-slate-700">
                  ...
                  <CardContent className="px-3 pb-2">
                    <Input type="number" value={takeoffAlt} onChange={e => setTakeoffAlt(e.target.value)}
                      className="bg-slate-700 border-slate-600 text-white text-xs h-7" placeholder="50" />
                  </CardContent>
                </Card>
                <Card className="bg-slate-800 border-slate-700">
                  <CardHeader className="pb-1 px-3 pt-2">
                  ...
                  <CardContent className="px-3 pb-2 space-y-1">
                    <div className="grid grid-cols-3 gap-1">
                      <div><label className="text-[10px] text-slate-400">纬度</label>
                        <Input type="number" step="0.0001" value={gotoLat} onChange={e => setGotoLat(e.target.value)}
                          className="bg-slate-700 border-slate-600 text-white text-xs h-6" /></div>
                      <div><label className="text-[10px] text-slate-400">经度</label>
                        <Input type="number" step="0.0001" value={gotoLon} onChange={e => setGotoLon(e.target.value)}
                          className="bg-slate-700 border-slate-600 text-white text-xs h-6" /></div>
                      <div><label className="text-[10px] text-slate-400">高度</label>
